...
<body class="bg-slate-950 text-slate-100 h-screen flex flex-col font-sans overflow-hidden">
    ...
    <!-- Üst Bar -->
    <header class="bg-slate-900 border-b border-slate-800 px-6 py-3.5 flex justify-between items-center shadow-xl z-20">
        <div class="flex items-center gap-3">
            <div class="w-10 h-10 rounded-2xl bg-indigo-600 flex items-center justify-center font-black text-white shadow-lg shadow-indigo-600/30">A</div>
            <div>
                <h1 class="text-sm font-black tracking-wider text-indigo-400">ALAGÖZ PANEL</h1>
                <p class="text-xs text-slate-400">ID'niz: <strong class="text-emerald-400 font-mono">{{ current_user.custom_id }}</strong> ({{ current_user.username }})</p>
            </div>
        </div>
    ...
    <div class="flex-1 flex flex-col md:flex-row overflow-hidden">
        ...
        <!-- Sol Taraf: Arama ve Üyeler -->
        <aside class="w-full md:w-80 bg-slate-900/40 border-r border-slate-800 flex flex-col p-4 gap-3">
            <div class="flex gap-2">
                <input type="text" id="search-id-input" placeholder="ID ile Ara (Örn: 482910)" class="flex-1 px-3.5 py-2.5 bg-slate-950 border border-slate-800 rounded-xl text-xs text-slate-200 focus:outline-none focus:border-indigo-500">
                <button onclick="searchUser()" class="px-4 py-2.5 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-xs font-bold transition shadow-md shadow-indigo-600/25">Bul</button>
            </div>
            ...
            <div id="users-list" class="flex-1 overflow-y-auto space-y-2 pr-1">
                <p class="text-xs text-slate-500 text-center mt-4">Kullanıcılar yükleniyor...</p>
            </div>
        </aside>

        <!-- Sağ Taraf: Sohbet ve Arama Ekranı -->
        <section class="flex-1 flex flex-col bg-slate-950/60 relative">
            <div id="chat-header" class="px-6 py-3.5 bg-slate-900 border-b border-slate-800 text-xs font-bold text-slate-300 hidden flex justify-between items-center z-10 shadow-sm">
                <span id="active-chat-title" class="flex items-center gap-2">Sohbet Seçilmedi</span>
                <button id="call-btn" onclick="toggleAudioCall()" class="px-4 py-2 bg-emerald-600 hover:bg-emerald-500 text-white rounded-xl text-xs font-bold transition shadow-lg shadow-emerald-600/20 flex items-center gap-1.5">📞 Sesli Aramayı Başlat</button>
            </div>

            <!-- Ses Akışı İçin Gizli Audio Elementi -->
            ...
            <div id="chat-messages" class="flex-1 p-6 overflow-y-auto space-y-3 flex flex-col">
                <div class="m-auto text-center space-y-2">
                    <div class="w-14 h-14 rounded-2xl bg-slate-900 border border-slate-800 flex items-center justify-center mx-auto text-xl shadow-inner">💬</div>
                    <p class="text-xs text-slate-400">Sol listeden bir üyeye tıkla ya da ID aratarak görüşmeye başla.</p>
                </div>
            </div>

            <form id="chat-form" onsubmit="sendPrivateMessage(event)" class="p-4 bg-slate-900 border-t border-slate-800 flex gap-3 hidden z-10 shadow-lg">
                <input type="text" id="message-input" autocomplete="off" placeholder="Mesajınızı yazın..." class="flex-1 px-4 py-3 bg-slate-950 border border-slate-800 rounded-xl text-sm focus:outline-none focus:border-indigo-500 text-slate-200 transition">
                <button type="submit" class="px-6 py-3 bg-indigo-600 hover:bg-indigo-500 text-white rounded-xl text-sm font-semibold transition shadow-md shadow-indigo-600/30">Gönder</button>
            </form>
        </section>
    </div>
    ...
    <script>
        ...
        function renderUsers(users) {
            ...
            users.forEach(u => {
                ...
                item.className = "p-3 rounded-xl bg-slate-900 border border-slate-800 cursor-pointer hover:bg-slate-800 transition flex items-center justify-between group";
                item.onclick = () => startChat(u.custom_id, u.username, u.online);
                item.innerHTML = `
                    <div class="flex items-center gap-3">
                        <div class="relative w-9 h-9 rounded-xl bg-indigo-500/20 border border-indigo-500/30 flex items-center justify-center font-bold text-indigo-400 text-xs">
                    ...
                    <span class="text-[10px] text-slate-400 bg-slate-950 px-2 py-1 rounded-lg border border-slate-800">Seç</span>
                `;
        ...
        function appendMessage(sender, msg, isMe) {
            ...
            div.innerHTML = `
                <div class="px-4 py-2.5 rounded-2xl max-w-xs text-sm shadow-md ${isMe ? 'bg-indigo-600 text-white rounded-br-none' : 'bg-slate-900 border border-slate-800 text-slate-200 rounded-bl-none'}">
                    ${msg}
                </div>
            `;
...
<body class="bg-slate-950 text-slate-100 flex items-center justify-center h-screen px-4 font-sans">
    <div class="bg-slate-900 border border-slate-800 p-8 rounded-3xl shadow-2xl w-full max-w-md">
        <div class="text-center mb-6">
            ...
            <p class="text-xs text-slate-400 mt-1">Güvenli İletişim Paneli</p>
        </div>
